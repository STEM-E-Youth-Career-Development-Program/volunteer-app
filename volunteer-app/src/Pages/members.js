...
import { collection, getDocs, doc, writeBatch } from "../index.js";
...
function MemberTable() {
    ...
    const [loading, setLoading] = useState(true); // loading state for onboarding table
        ...
        const fetchMembers = async () => {
            try {
                ...
                const membersData = membersSnapshot.docs.map(doc => ({
                    id: doc.id,
                    ...doc.data()
                })); // Map through docs and docs id to get data
    ...
    const updateWaiver = async () => {
        ...
        const updatedData = members.map(row => {
            const match = checkList.find(k => k[3] === row.email);
            return match ? { ...row, signedWaiver: match[15] } : row;
        });
    ...
    // Members who have not signed the waiver
